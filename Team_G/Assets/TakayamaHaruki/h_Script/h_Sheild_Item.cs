...
public class Sheild_Item : ItemBase
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "Item")
        {
            ...
            //反射スピード
            if (collision.GetComponent<Item>().item_id == reflect_speed_item)
            {
                if (item_count[reflect_speed_item] < collision.GetComponent<Item>().max_item_count)
                {
                    //反射スピードup
                    enemy_ref.GetComponent<g_enemy>().speed += up_reflect_speed;
                    Debug.Log(enemy_ref.GetComponent<g_enemy>().speed);
                    item_count[reflect_speed_item]++;
                }
            }
